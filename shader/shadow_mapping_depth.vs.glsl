...
void main()
{
    ...
    // 将if...else改为trick写法
    gl_Position = int(render_point_cloud) * light_space_matrix * model * a_instance_matrix * vec4(a_pos, 1.0)
        + int(!render_point_cloud) * light_space_matrix * model * vec4(a_pos, 1.0);
}